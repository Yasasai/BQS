    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-gray-900">
            ...
            <div className="flex-1 px-8 py-6 max-w-[1600px] mx-auto w-full">
                ...
                {/* Optimized Toolbar */}
                <div className="flex items-center justify-between bg-white p-4 rounded-t-lg border border-gray-200 border-b-0">
                ...
                <div className="bg-white border border-gray-200 rounded-b-lg overflow-x-auto shadow-sm">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {loading ? (
                                <tr><td colSpan={11} className="px-6 py-12 text-center text-gray-500">Loading pipeline data...</td></tr>
                            ) : filteredOpportunities.length === 0 ? (
                                <tr><td colSpan={11} className="px-6 py-12 text-center text-gray-500">No opportunities found in this view.</td></tr>
                            ) : (
                                filteredOpportunities.map((opp) => (
                                    <tr key={opp.id} className="hover:bg-gray-50 transition-colors group cursor-pointer text-[13px] text-[#333333]" onClick={() => navigate(`/opportunity/${opp.id}`)}>
                                        ...
                                        <td className="px-4 py-4">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium ${opp.workflow_status === 'COMPLETED' ? 'bg-[#E8F5E9] text-[#2E7D32]' :
                                                    opp.workflow_status === 'SUBMITTED_FOR_REVIEW' ? 'bg-red-50 text-red-700' :
                                                        'bg-[#E8F5E9] text-[#2E7D32]'
                                                }`}>
                                                {opp.workflow_status === 'NEW' ? 'Committed' : (opp.workflow_status || 'Committed')}
                                            </span>
                                        </td>
                                        <td className="px-4 py-4 text-[#666666]">
