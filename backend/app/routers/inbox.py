...
def assign_opportunity(data: AssignInput, db: Session = Depends(get_db)):
    ...
    # Only create a new version if the latest one is already finalized (APPROVED/REJECTED).
    # Otherwise, let the new assignee join the existing active version.
    ...
    if last_ver and last_ver.status in ['APPROVED', 'REJECTED']:
        # Finalized round, start a fresh one for the new assignee
        new_ver_no = last_ver.version_no + 1
        current_version = OppScoreVersion(
            opp_id=data.opp_id,
            ...
            summary_comment=f"New round started after reassignment.",
            attachment_name=last_ver.attachment_name
        )
        db.add(current_version)
        db.flush()
        
        # Clone previous values as a baseline
        for ov in old_values:
            db.add(OppScoreSectionValue(
                score_version_id=current_version.score_version_id,
                section_code=ov.section_code,
                score=ov.score,
                notes=ov.notes,
                selected_reasons=ov.selected_reasons
            ))
    elif last_ver:
        # Reuse existing unfinished version
        current_version = last_ver
        # RESET submission flags for the new round of input
        current_version.sa_submitted = False
        current_version.sp_submitted = False
        if current_version.status in ['SUBMITTED', 'NOT_STARTED']: 
            current_version.status = 'ASSIGNED'
    else:
        # No version exists at all, create V1
        current_version = OppScoreVersion(
            opp_id=data.opp_id,
            version_no=1,
            status="ASSIGNED",
