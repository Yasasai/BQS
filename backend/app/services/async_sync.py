...
def bulk_upsert(items):
    ...
    # init_db() # Assuming DB is initialized by main app now
    db = SessionLocal()
    saved_count = 0
...
async def run_async_sync():
    """
    Main entry point for async sync, callable from FastAPI.
    """
    start_time = datetime.now(timezone.utc)
    log("🚀 Starting Async Batch Sync from Service...")
    ...
    log(f"🎉 Sync Finished in {duration:.2f}s")
    return {"status": "success", "total": total, "duration": duration}
